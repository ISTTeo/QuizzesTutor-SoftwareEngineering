...
class CreateTournamentsServiceSpockPerformanceTest extends Specification {
    ...
    def "performance testing to get 1000 tournaments"() {
        ...
        when:
        1.upto(1, {
            tournamentDto.setName(TOURNAMENT_NAME+ it)
            tournamentDto.setTopics(Collections.singletonList(new TopicDto(topic)))
            tournamentService.createTournament(courseExecution.getId(), tournamentDto, user.getId())
        })
