...
public class Question implements DomainEntity {
    ...
    public Question(QuestionProposal proposal) {
        setTitle(proposal.getTitle());
        setKey(proposal.getKey());
        setContent(proposal.getContent());
        setCreationDate(null); // now
        setCourse(proposal.getCourse());

        if (proposal.getImage() != null)
            setImage(proposal.getImage());

        int index = 0;
        for (Option option : proposal.getOptions()) {
            Option opt = new Option(option);
            opt.setSequence(index++);
            opt.setQuestion(this);
        }

        for (Topic topic : proposal.getTopics())
            addTopic(topic);
    }
