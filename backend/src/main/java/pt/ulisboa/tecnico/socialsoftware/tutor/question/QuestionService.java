...
public class QuestionService {
    ...
    public QuestionDto createQuestion(int courseId, QuestionDto questionDto) {
        ...
        question.getOptions().stream().forEach(opt -> optionRepository.save(opt));
    ...
    @Retryable(
      ...
      backoff = @Backoff(delay = 5000))
    @Transactional(isolation = Isolation.REPEATABLE_READ)
    public QuestionDto createQuestionFromProposal(QuestionProposal proposal) {
        Question question = new Question(proposal);
        questionRepository.save(question);
        question.getOptions().stream().forEach(opt -> optionRepository.save(opt));
        return new QuestionDto(question);
    }
    ...
    public QuestionDto updateQuestion(Integer questionId, QuestionDto questionDto) {
        ...
        removeMissingOptions(question, questionDto);
    ...
    private void removeMissingOptions(Question orig, QuestionDto replacement) {
