...
public class TutorPermissionEvaluator implements PermissionEvaluator {
        ...
        if (targetDomainObject instanceof CourseDto) {
            ...
            switch (permissionValue) {
                case "EXECUTION.CREATE":
                    return userService.getEnrolledCoursesAcronyms(userId)
                            .contains(courseDto.getAcronym() + courseDto.getAcademicTerm());
                case "DEMO.ACCESS":
        ...
        if (targetDomainObject instanceof Integer) {
            ...
            switch (permissionValue) {
                ...
                case "ASSESSMENT.ACCESS":
                    return userHasThisExecution(userId,
                            assessmentService.findAssessmentCourseExecution(id).getCourseExecutionId());
                case "QUIZ.ACCESS":
                    return userHasThisExecution(userId, quizService.findQuizCourseExecution(id).getCourseExecutionId());
                case "TOURNAMENT.ACCESS":
                    return userHasThisExecution(userId,
                            tournamentService.findTournamentCourseExecution(id).getCourseExecutionId());
                case "DISCUSSION.ACCESS":
                    return userHasAnExecutionOfTheCourse(userId,
                            discussionService.findDiscussionCourse(id).getCourseId());
                default:
                    return false;
            }
        }
    ...
    private boolean userHasAnExecutionOfTheCourse(int userId, int id) {
        return userService.getCourseExecutions(userId).stream().anyMatch(course -> course.getCourseId() == id);
    }

    private boolean userHasThisExecution(int userId, int id) {
        return userService.getCourseExecutions(userId).stream().anyMatch(course -> course.getCourseExecutionId() == id);
    }
