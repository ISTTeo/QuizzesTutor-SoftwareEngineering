...
public class QuizService {
    ...
    @Retryable(
      ...
      backoff = @Backoff(delay = 5000))
    @Transactional(isolation = Isolation.REPEATABLE_READ)
    public QuizDto createQuizWithTopics(int executionId, QuizDto quizDto, Set<Topic> topics, User user){
        CourseExecution courseExecution = courseExecutionRepository.findById(executionId).orElseThrow(() -> new TutorException(COURSE_EXECUTION_NOT_FOUND, executionId));
        ...
        if (availableQuestions.size() < quizDto.getNumberOfQuestions()) {
            throw new TutorException(NOT_ENOUGH_QUESTIONS);
        }

        for (Question q : user.filterQuestionsByStudentModel(quizDto.getNumberOfQuestions(), availableQuestions.stream().collect(Collectors.toList()))){
