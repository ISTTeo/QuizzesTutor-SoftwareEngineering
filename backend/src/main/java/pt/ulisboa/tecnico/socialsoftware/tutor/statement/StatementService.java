...
public class StatementService {
    ...
    @Retryable(
      ...
      backoff = @Backoff(delay = 5000))
    @Transactional(isolation = Isolation.REPEATABLE_READ)
    public List<StatementQuizDto> getOngoingTournaments(int userId, int executionId) {
        User user = userRepository.findById(userId).orElseThrow(() -> new TutorException(USER_NOT_FOUND, userId));
        ...
        // create QuizAnswer for quizzes
        user.getEnrolledIn().stream().filter(t->t.getQuiz() != null)
                .filter(t -> t.getQuiz().getType().equals(Quiz.QuizType.TOURNAMENT))
                .filter(t -> t.getQuiz().getAvailableDate() == null || t.getQuiz().getAvailableDate().isBefore(now))
                .filter(t -> !studentQuizIds.contains(t.getQuiz().getId()))
                .forEach(t ->  {
                    if (t.getQuiz().getConclusionDate() == null || t.getQuiz().getConclusionDate().isAfter(now)) {
                        QuizAnswer quizAnswer = new QuizAnswer(user, t.getQuiz());
                        quizAnswerRepository.save(quizAnswer);
                    }
                });
    ...
    @Retryable(
      ...
      backoff = @Backoff(delay = 5000))
    @Transactional(isolation = Isolation.REPEATABLE_READ)
    public List<SolvedQuizDto> getSolvedTournaments(int userId, int executionId){
        User user = userRepository.findById(userId).orElseThrow(() -> new TutorException(USER_NOT_FOUND, userId));
        ...
        return user.getQuizAnswers().stream()
                ...
                .filter(quizAnswer -> quizAnswer.isCompleted())
                .filter(quizAnswer -> quizAnswer.getQuiz().getCourseExecution().getId() == executionId)
                .filter(quizAnswer -> quizAnswer.getQuiz().getConclusionDate() == null || DateHandler.now().isAfter(quizAnswer.getQuiz().getConclusionDate()))
    ...
    @Retryable(
      ...
      backoff = @Backoff(delay = 5000))
    @Transactional(isolation = Isolation.REPEATABLE_READ)
    public List<SolvedQuizDto> getSolvedOngoingTournaments(int userId, int executionId) {
        User user = userRepository.findById(userId).orElseThrow(() -> new TutorException(USER_NOT_FOUND, userId));
